...
|---------|---------------------------|--------|
| **Customer portal** | Always **Authentik OIDC** (invite-only; cookie `cp_oidc_session`) | Never advertise as public. Access is **off** on `client.cronnecture.com` (`skip_access`) so Stripe return URLs do not 1043 |
