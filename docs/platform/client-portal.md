...
Authentik-gated customer hub at:
...
3. Unauthenticated visit → Authentik sign-in (that **is** account setup for the invited email). Strangers cannot enter; they see `/auth/not-invited`.
4. After OIDC, they land **in** the portal. Control plane persists **account ↔ portal** (`Client.contact_email` + `Client.logto_sub`).
...
- Legacy path-scoped Access apps (`client-{slug}-customer-portal`) are removed on portal sync — **do not** put Cloudflare Access in front of this hub (Stripe return URLs would 1043)
...
Platform DNS/tunnel: `cf_portals.yml` entry `Client customer portal` (`skip_access: true` — Authentik OIDC gates the hub in the control plane).
