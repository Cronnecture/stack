...
class InfrastructureManager:
    ...
    async def deploy_client(
        ...
        if namespace.removeprefix("client-") in PROTECTED_NAMESPACES or namespace in PROTECTED_NAMESPACES:
...
"""
    ...
    async def cluster_snapshot(self) -> Dict[str, Any]:
        ...
        for ns in self.core.list_namespace().items:
            ...
            pods = self.core.list_namespaced_pod(name)
            running = sum(1 for p in pods.items if p.status.phase == "Running")
            namespaces.append({"name": name, "pods": len(pods.items), "running": running})
