...
Vaultwarden onto empty in-cluster Postgres, or Logto onto identity-postgres
when the recovered data lives on logto-postgres.
...
def main() -> int:
    ...
    logto_dsn = pod_printenv("identity", "logto", "DB_URL")
    ...
    if not vault_dsn or not auth_host or not logto_dsn or not auth_password:
        raise SystemExit("could not read live identity database settings from running pods")

    patch_secret(
        {
            "postgres-dsn-vaultwarden": vault_dsn,
            "postgres-dsn-logto": logto_dsn,
    ...
    print(f"  logto       -> {host_of_dsn(logto_dsn)}")
